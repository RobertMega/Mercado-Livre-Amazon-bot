  ...
  const client = createWhatsappApiClient({
    ...
    fetchImpl: async (url, init) => {
      ...
      return {
        ok: true,
        async json() {
          return { success: true, messageId: 'image-1' }
        },
      }
    },
  })
...
test('whatsapp api client rejects successful HTTP responses without a message id', async () => {
  const client = createWhatsappApiClient({
    baseUrl: 'http://localhost:3000',
    fetchImpl: async () => ({
      ok: true,
  ...
  await assert.rejects(
    ...
    /without a message id/i,
  )
})

test('whatsapp api client keeps API error messages when send fails', async () => {
  const client = createWhatsappApiClient({
    baseUrl: 'http://localhost:3000',
    fetchImpl: async () => ({
      ok: false,
      status: 500,
      async json() {
        return { error: 'Session "teclado" is not connected yet' }
      },
    }),
  })
