...
function isValidWhatsappRecipient(value) {
  ...
  if (normalized.includes('@')) {
    return /^[0-9]{5,30}@(g\.us|s\.whatsapp\.net)$/i.test(normalized)
  }

  return /^[0-9]{8,20}$/.test(normalized.replace(/\D/g, ''))
}
...
export async function sendText(req, reply, whatsapp = defaultWhatsappService) {
  ...
  if (!isValidWhatsappRecipient(Phone)) {
    return reply.code(400).send({ error: 'Field "Phone" must be a WhatsApp phone number or JID' })
  }
